...
def calcula_equilibrio_financeiro():
    gastos_essenciais = Valores.objects.filter(
        ...
        tipo='S').filter(categoria__essencial=True)
    gastos_nao_essenciais = Valores.objects.filter(
        data__month=datetime.now().month).filter(
        tipo='S').filter(categoria__essencial=False)
    
    total_gastos_esseciais = calcula_total(gastos_essenciais, 'valor')
    total_gastos_nao_essenciais = calcula_total(gastos_nao_essenciais, 'valor')

    total = total_gastos_esseciais + total_gastos_nao_essenciais

    try:
        percentual_gastos_essenciais = (total_gastos_esseciais * 100) / total
